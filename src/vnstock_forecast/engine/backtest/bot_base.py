...
class BotBase(ABC):
    ...
    Trong ``on_step``, bot truy cập qua ctx::
        ...
        ctx.symbols           danh sách symbols (primary resolution)
        ctx.resolutions       danh sách resolutions có trong data
        ctx.primary_resolution  resolution đang lái vòng lặp
        ctx.price(sym)        giá Close hiện tại (primary resolution)
        ctx.history(sym, n)             n bars gần nhất (primary resolution)
        ctx.history(sym, n, resolution) n bars gần nhất ở resolution chỉ định
        ctx.latest(sym)               bar mới nhất (primary resolution)
        ctx.latest(sym, resolution)   bar mới nhất ở resolution chỉ định
