...
class CloseReason(Enum):
    ...
    TIME_LIMIT = "time_limit"  # Hết thời hạn giữ lệnh (max_holding_days)
...
class Position:
    ...
    """Số ngày giao dịch tối đa giữ lệnh. ``None`` = không giới hạn."""
    ...
    def is_time_expired(self, current_time: datetime) -> bool:
        """
        Kiểm tra đã vượt ``max_holding_days`` ngày giao dịch kể từ ngày mua chưa.
...
class Portfolio:
    ...
    def close_position(
        ...
        """
        Đóng vị thế (toàn bộ hoặc một phần). Cộng tiền = (giá bán × số lượng) − phí.

        Tuân thủ luật T+N: vị thế phải đủ ``settlement_days`` ngày giao
        dịch kể từ ngày mua mới được bán (trừ ``END_OF_DATA``).

        Args:
            quantity: Số lượng muốn bán. ``None`` / 0 = bán toàn bộ.
                      Nếu nhỏ hơn ``pos.quantity``, vị thế gốc được
                      tách làm hai: phần bán bị đóng, phần còn lại
                      tiếp tục mở với cùng ``id``.

        Raises:
            KeyError: ``position_id`` không tồn tại hoặc đã đóng.
            ValueError: Chưa đủ T+N ngày giao dịch, hoặc quantity không hợp lệ.
        """
        ...
        # Xác định số lượng thực bán
        sell_qty = quantity if (quantity and quantity > 0) else pos.quantity
        if sell_qty > pos.quantity:
            raise ValueError(
                f"Số lượng bán ({sell_qty:,.0f}) vượt quá vị thế "
                f"({pos.quantity:,.0f}) của '{pos.symbol}'."
            )

        if sell_qty < pos.quantity:
            # Bán một phần: tách vị thế gốc
            remaining_qty = pos.quantity - sell_qty
            pos.quantity = sell_qty  # phần bị đóng
            # Tạo vị thế mới cho phần còn lại (giữ nguyên id _open)
            remaining = Position(
                id=uuid.uuid4().hex[:8],
                symbol=pos.symbol,
            ...
            # Cập nhật danh sách mở: thay vị thế gốc bằng phần còn lại
            self._open[position_id] = remaining
        else:
            # Bán toàn bộ: xóa khỏi _open
    ...
    def check_sl_tp(
        ...
        Luật T+N: chỉ trigger SL/TP/TIME_LIMIT khi vị thế đã đủ ngày settlement.
        """
        ...
        for pos in list(self._open.values()):
            ...
            if pos.stop_loss is not None and low <= pos.stop_loss:
                ...
                # Đóng theo Close của bar hiện tại
                closed.append(
                    self.close_position(
                        pos.id, close, timestamp, CloseReason.TIME_LIMIT
                    )
                )
